...
int create_gc_announce_request(uint8_t *packet, uint16_t max_packet_length, const uint8_t *dest_client_id,
                               ...
                               const uint8_t *gc_data, int16_t gc_data_length, bool fuzz)
{
    ...
    if (fuzz) {
        int rand = random_int_range(2);

        if (rand == 0) {
            encrypted_size = fuzz_packet(plain, encrypted_size, random_int_range(2), sizeof(plain), ONION_PING_ID_SIZE
                                         + CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_PUBLIC_KEY_SIZE);
        } else {
            encrypted_size = fuzz_packet(plain, encrypted_size, random_int_range(2), sizeof(plain), ONION_PING_ID_SIZE + CRYPTO_PUBLIC_KEY_SIZE
                                         + CRYPTO_PUBLIC_KEY_SIZE + sizeof(sendback_data));
        }
    }
...
static int handle_gc_announce_request(Onion_Announce *onion_a, IP_Port source, const uint8_t *packet, uint16_t length)
{
    ...
    GC_Announce gc_announces[MAX_SENT_ANNOUNCES];
    ...
    /* Start fuzz code */
    uint8_t orig_pl[sizeof(pl)];
    memcpy(orig_pl, pl, sizeof(pl));
    size_t orig_offset = offset;

    for (int i = 0; i < 20; ++i) {
        int rand = random_int_range(4);

        if (rand == 0) {
            offset = fuzz_packet(pl, offset, random_int_range(2), sizeof(pl), 0);
        } else if (rand == 1) {
            offset = fuzz_packet(pl, offset, random_int_range(2), sizeof(pl), 1 + ONION_PING_ID_SIZE);
        } else if (rand == 2) {
            offset = fuzz_packet(pl, offset, random_int_range(2), sizeof(pl), 1 + ONION_PING_ID_SIZE + 1);
        } else if (rand == 3) {
            if (announces_length) {
                offset = fuzz_packet(pl, offset, random_int_range(2), sizeof(pl), 1 + ONION_PING_ID_SIZE + 1 + nodes_length);
            }
        }

        uint8_t fuzz_data[ONION_ANNOUNCE_RESPONSE_MAX_SIZE];
        len = encrypt_data_symmetric(shared_key, nonce, pl, offset,
                                     fuzz_data + 1 + ONION_ANNOUNCE_SENDBACK_DATA_LENGTH + CRYPTO_NONCE_SIZE);

        if (len != offset + CRYPTO_MAC_SIZE) {
            continue;
        }

        memcpy(pl, orig_pl, sizeof(pl));
        offset = orig_offset;

        fuzz_data[0] = NET_PACKET_ANNOUNCE_RESPONSE;
        memcpy(fuzz_data + 1, plain + ONION_PING_ID_SIZE + CRYPTO_PUBLIC_KEY_SIZE + CRYPTO_PUBLIC_KEY_SIZE,
               ONION_ANNOUNCE_SENDBACK_DATA_LENGTH);
        memcpy(fuzz_data + 1 + ONION_ANNOUNCE_SENDBACK_DATA_LENGTH, nonce, CRYPTO_NONCE_SIZE);

        if (send_onion_response(onion_a->net, source, fuzz_data,
                                1 + ONION_ANNOUNCE_SENDBACK_DATA_LENGTH + CRYPTO_NONCE_SIZE + len,
                                packet + (length - ONION_RETURN_3)) == -1) {
            continue;
        }
